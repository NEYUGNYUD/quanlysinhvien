...
@section('content')
    <div class="col-md-12">
        ...
        <form id="summary-form" action=" {{ route('classes.update', ['class' => $class->slug]) }}" class="form-horizontal" method="post">
            {{ csrf_field() }}
            <section class="panel">
                        ...
                        <div class="col-sm-6">
                            <select class="form-control" name="faculty">
                                @foreach($facultiesList as $faculty)
                                    <option value="{{ $faculty->id }}" {{ $class->
                                        faculty_id == $faculty->id?'selected':""
                                    }}>{{ $faculty->name }}</option>
                                @endforeach
                            </select>
                        </div>
